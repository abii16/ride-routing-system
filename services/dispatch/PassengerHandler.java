...
/**
 * Passenger Handler - Handles individual passenger connections in the Dispatch Server
 */
public class PassengerHandler implements Runnable {
    ...
    private void handleRegister(Message message) {
        ...
        if (dbResponse != null && dbResponse.getPayloadBoolean("success")) {
            ...
            response.addPayload("error", dbResponse != null ? 
                              dbResponse.getPayloadString("error") : "Registration failed");
            sendMessage(response);
        }
    }
    ...
    private void handleRideRequest(Message message) {
        ...
        System.out.println("[PassengerHandler] Processing ride request for " + username);
        ...
        String result = DispatchServer.assignNearestDriver(username, pickupLat, pickupLon, destLat, destLon, startAddr, destAddr);
        ...
        if (result != null) {
            ...
            if (parts[0].equals("WAITING")) {
                ...
                // Send "Waiting for Driver" response (Using RIDE_ASSIGNMENT message but with status)
                ...
                // Easier: Use NO_DRIVERS_AVAILABLE but include rideId so client knows it is QUEUED.
                
                Message response = new Message(MessageType.RIDE_REQUEST); // Echo back request as pending?
                // Or better: Use a new status or just standard NO_DRIVERS_AVAILABLE but success=true?
                ...
                System.out.println("[PassengerHandler] Ride assigned: " + driverUsername + " to " + username);
            }
        } else {
            ...
            System.out.println("[PassengerHandler] Failed to create ride for " + username);
        }
    }
    ...
    private void handleManualAssign(Message message) {
        ...
        if(!success) response.addPayload("error", "Assignment failed (Driver might be missing or busy)");
        sendMessage(response);
    }
