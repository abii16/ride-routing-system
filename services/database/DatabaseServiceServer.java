...
class DatabaseRequestHandler implements Runnable {
    ...
    public void run() {
        try {
            ...
            System.out.println("[DatabaseRequestHandler] Received: " + requestJson.substring(0, Math.min(requestJson.length(), 100)) + "...");
            ...
            // --- SYNCHRONIZATION ---
            // If modification was successful AND type is modification AND not already a sync message
    ...
    private void syncToPeers(Message originalRequest) {
        if (syncHosts == null || syncHosts.isEmpty()) return;
